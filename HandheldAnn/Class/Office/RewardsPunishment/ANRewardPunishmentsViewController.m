//
//  ANRewardPunishmentsViewController.m
//  HandheldAnn
//
...
@interface ANRewardPunishmentsViewController ()

@end

@implementation ANRewardPunishmentsViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor=[UIColor whiteColor];
    self.title=@"奖罚结果";
    NSArray * titleArray=@[@"预计奖罚表",@"最终奖罚表",@"未出货原因表"];
    /**< 每个标题下对应的控制器，只需将您创建的控制器类名加入下列数组即可(注意:数量应与上方的titles数量保持一致，若少于titles数量，下方会打印您缺少相应的控制器，同时默认设置的最大控制器数量为10=。=)  。
     **/
    NSArray *vcsArray = @[
                          @"ANExpectViewController",
                          @"ANFinalRPViewController",
                          @"ANReasonViewController",
                          ];
    NSArray *colorArray = @[
...
//    [[NinaPagerView alloc]initWithTitles:titleArray WithVCs:vcsArray WithColorArrays:colorArray];
