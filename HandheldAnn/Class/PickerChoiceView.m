...
- (void)setArrayType:(ARRAYTYPE)arrayType
{
    ...
    self.selectLb.text = self.selectStr;
    if (arrayType==StartTimeArray) {
        [self creatDate];
    } else if (arrayType==EndTimeArray) {
        [self creatDate];
    }else if (arrayType==ShowArray) {
        NSArray *arr = [NSArray arrayWithObjects:@"省区",@"大区",@"分拨",@"片区",@"网点",@"门店", nil];
        [self.array addObject:arr];
    }else if (arrayType==WangDianArray) {
        NSArray *_arr=[NSArray arrayWithObjects:@"全网",@"网络",@"市场", nil];
        [self.array addObject:_arr];
    }else {
        if (arrayType==ProvinceArray) {
            paraDic=@{@"action":@"getList",@"Field":@"REGION"};
        }else if (arrayType==RegionalArray) {//大区
            if ([self.superiorStr1 isEqualToString:@"请选择"]) {
                paraDic=@{@"action":@"getList",@"Field":@"PROVINCE"};
            } else {
                self.superiorStr1 = [NSString stringWithFormat:@"and+REGION %@",self.superiorStr1];
                NSLog(@"==%@",self.superiorStr1);
                paraDic=@{@"action":@"getList",@"Field":@"PROVINCE",@"Where2":self.superiorStr1};
            } 
        }else if (arrayType==Distribution) {//分拨
            if ([self.superiorStr1 isEqualToString:@"请选择"]) {
                if ([self.superiorStr2 isEqualToString:@"请选择"]) {
                    paraDic=@{@"action":@"getList",@"Field":@"ALLOCATE"};
                }else {
                    self.superiorStr1 = [NSString stringWithFormat:@"and+PROVINCE %@",self.superiorStr1];
                    NSLog(@"==%@",self.superiorStr1);
                    paraDic=@{@"action":@"getList",@"Field":@"ALLOCATE",@"Where2":self.superiorStr1};
                }
            }
        }else if (arrayType==AreaArray) {
            
        }else if (arrayType==BranchesArray) {
            
        }else if (arrayType==StoresArray) {
            
        }
        [NetWorkRequest getWithExtendUrl:@"SiteList.ashx" Para:paraDic Complete:^(BOOL succeed, id Object) {
            if ([[NSString stringWithFormat:@"%@",[Object objectForKey:@"state"]] isEqualToString:@"1"]) {
                _arrayData=[Object objectForKey:@"returninfo"];
                for (int i=0; i<_arrayData.count; i++) {
                    NSString *rate=[[_arrayData objectAtIndex:i ] objectForKey:@"NAME"];
                    [dataArray addObject:rate];
                }
                [self.array addObject:dataArray];
                NSLog(@"array=%@",self.array);
                // 将局部变量dataArray作为参数传出去
//                [self performSelectorOnMainThread:@selector(getDataList:) withObject:dataArray waitUntilDone:YES];
            }
        }];
    }
}
...
- (void)getDataList:(id)sender {
    ...
    [self.array addObject:dataList]; // 保留原来的_dataList, 用在这里
    NSLog(@"dataArray===%@  %li self.array=%@  superiorStr=%@",dataArray,(unsigned long)dataArray.count,self.array,self.superiorStr1);
}
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component{
    ...
    if (self.arrayType == StartTimeArray) {
...
- (CGFloat)pickerView:(UIPickerView *)pickerView widthForComponent:(NSInteger)component
{
    ...
    if (self.arrayType == StartTimeArray||self.arrayType == EndTimeArray) {
